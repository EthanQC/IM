...
// Execute 用旧 RefreshToken 颁发新一对 Access/Refresh Token
func (uc *RefreshTokenUseCase) Execute(ctx context.Context, oldJTI string) (string, string, error) {
	rec, err := uc.RefreshRepo.Find(ctx, oldJTI)
	if err != nil {
		return "", "", fmt.Errorf("查询 RefreshToken 失败: %w", err)
	}
	if rec == nil || rec.IsRevoked {
		return "", "", fmt.Errorf("无效或已撤销的 RefreshToken")
	}
	if time.Now().After(rec.RefreshExpiresAt) {
		return "", "", fmt.Errorf("RefreshToken 已过期")
	}
	...
	if status != nil && !status.IsActive() {
		return "", "", fmt.Errorf("用户已被禁用: %s", rec.UserID)
	}

	if err := uc.RefreshRepo.Revoke(ctx, oldJTI); err != nil {
		return "", "", fmt.Errorf("撤销旧 RefreshToken 失败: %w", err)
	}
